...
export interface CodexResult { process: ProcessResult; result: WorkerResult | ReviewResult | null; usage: Usage; error?: string }
...
export async function runCodex(options: CodexOptions): Promise<CodexResult> {
  ...
  if (result.code !== 0 || result.timedOut || result.aborted) return { process: result, result: null, usage, error: result.aborted ? 'Cancelled' : result.timedOut ? 'Execution timed out' : 'Codex exited unsuccessfully; inspect local bounded log' };
