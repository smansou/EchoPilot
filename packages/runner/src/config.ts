...
export interface Config extends RunnerConfig {
 port:number;maxTicketsPerRun:number;workerTimeoutMs:number;checkTimeoutMs:number;
...
export async function loadProject(root:string):Promise<{config:Config;tickets:Ticket[]}> {
 ...
 for(const key of ['port','maxTicketsPerRun','workerTimeoutMs','checkTimeoutMs','maxObservedTokensPerRun'] as const)if(!Number.isSafeInteger(config[key])||config[key]<1)throw new Error(`Invalid config.${key}`);
 ...
 config.allowedPaths??={};config.ticketChecks??={};
