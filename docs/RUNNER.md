...
- GPT-5.6 Terra, medium: current security tracer implementation while the stronger route is repeatedly stalling.
- GPT-5.6 Luna, medium: bounded independent reviews; substantive findings still trigger implementation repair.
- GPT-6 Astra: never selected by default; a deliberate config override is required.

Defaults: two active tickets, one verification/review lane, three implementation attempts per ticket, six starts per server run, 30-minute model invocation timeout, five-minute command timeout. Infrastructure stalls do not consume implementation attempts. Failed code checks or review can trigger a fresh-context repair. Authentication, unavailable models, permissions, ownership violations, Git conflicts, and publication failures stop scheduling for inspection. No silent model substitution occurs.
...
Ordinary worker, dependency-setup, ownership, check, and review failures are automatically retried in a fresh context without pausing unrelated tickets. A retry receives shared `repairPaths` only when the recorded failure actually concerns dependency setup, the lockfile, or a package manifest; normal code/review repairs remain inside ticket ownership and can run concurrently. A ticket is escalated only for a genuine human blocker such as authentication/credentials, unavailable hardware or user interaction, unsafe or dirty Git state, publication recovery, or a protected decision. Exhausted retries leave that ticket in **Needs attention** while independent work continues; they do not stop the scheduler.
...
A worker or reviewer that emits no structured activity for `idleTimeoutMs` is stopped, its worktree is retained, and the stall is treated as an infrastructure failure rather than consuming the ticket's implementation retry allowance. Each structured event resets the inactivity deadline; the separate 30-minute execution deadline still bounds workers that remain active. Repair prompts include the preceding coordinator/reviewer diagnosis, cap broad inspection, and direct the worker to edit after at most two targeted inspection commands. Execution/check/review failures get a bounded fresh-context retry; exhausted or externally blocked tickets are parked while independent tickets continue. Quota/authentication and shared Git integrity failures pause the run. Shared manifests are reserved only for diagnosed manifest/dependency repair, so unrelated ticket retries remain parallel. The token scheduling ceiling drains active work rather than killing a nearly finished candidate. Prior work and evidence remain available to retries.
