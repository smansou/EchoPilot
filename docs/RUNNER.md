...
Quiet periods now update the dashboard without terminating reasoning. The 30-minute execution deadline still applies. Execution/check/review failures get a bounded fresh-context retry; exhausted or externally blocked tickets are parked while independent tickets continue. Quota/authentication and shared Git integrity failures pause the run. Network retries consume the ordinary retry allowance. Shared manifest reservations apply only to actual implementation repair attempts, so previous quota failures no longer serialize all workers. The token scheduling ceiling drains active work rather than killing a nearly finished candidate. Prior work and evidence remain available to retries.
