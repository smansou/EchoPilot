...
A worker or reviewer that emits no structured activity for `idleTimeoutMs` is stopped, its worktree is retained, and the stall is treated as an infrastructure failure rather than consuming the ticket's implementation retry allowance. Each structured event resets the inactivity deadline; the separate 30-minute execution deadline still bounds workers that remain active. Execution/check/review failures get a bounded fresh-context retry; exhausted or externally blocked tickets are parked while independent tickets continue. Quota/authentication and shared Git integrity failures pause the run. Shared manifest reservations apply only to actual implementation repair attempts, so previous provider failures no longer serialize all workers. The token scheduling ceiling drains active work rather than killing a nearly finished candidate. Prior work and evidence remain available to retries.
